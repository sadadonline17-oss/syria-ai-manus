    ...
    >
      <View style={{ paddingHorizontal: 16, marginBottom: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        <TouchableOpacity
          ...
          style={{ flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: IMPERIAL.accent, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 10, borderWidth: 1, borderColor: IMPERIAL.border }}
        >
        ...
        <View style={{ alignItems: 'flex-end' }}>
          ...
          <Text style={{ fontSize: 12, color: IMPERIAL.textTertiary }}>إدارة المستودعات والأكواد</Text>
        </View>
      </View>
          ...
          >
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                ...
                <Text style={{ fontSize: 11, color: repo.status === 'synced' ? IMPERIAL.success : repo.status === 'ahead' ? IMPERIAL.success : IMPERIAL.warning }}>
                  {repo.status === 'synced' ? 'متزامن' : repo.status === 'ahead' ? `+${repo.ahead}` : `-${repo.behind}`}
                </Text>
              </View>
              <View style={{ alignItems: 'flex-end' }}>
                <Text style={{ fontSize: 14, fontWeight: '600', color: IMPERIAL.text }}>{repo.name}</Text>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 4 }}>
                  <Text style={{ fontSize: 10, color: IMPERIAL.textTertiary }}>{repo.lang}</Text>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
                    <Text style={{ fontSize: 11, color: IMPERIAL.textSecondary }}>{repo.branch}</Text>
      ...
      <View style={{ paddingHorizontal: 16, marginBottom: 20 }}>
        ...
        <View style={{ backgroundColor: IMPERIAL.card, borderRadius: 12, borderWidth: 1, borderColor: IMPERIAL.border, overflow: 'hidden' }}>
          {DEMO_CHANGES.map((change, i) => (
            ...
            >
              <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: change.status === 'added' ? IMPERIAL.git.added : change.status === 'modified' ? IMPERIAL.git.modified : IMPERIAL.git.deleted }} />
              <Text style={{ fontSize: 10, color: change.status === 'added' ? IMPERIAL.git.added : change.status === 'modified' ? IMPERIAL.git.modified : IMPERIAL.git.deleted, fontWeight: '600', width: 50 }}>
                {change.status === 'added' ? 'جديد' : change.status === 'modified' ? 'معدّل' : 'محذوف'}
              </Text>
              <View style={{ flex: 1 }} />
              <Text style={{ fontSize: 12, color: IMPERIAL.text }} numberOfLines={1}>{change.file}</Text>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}>
          ...
          >
            <GitCommitIcon size={14} color={commitMsg.trim() ? IMPERIAL.primaryForeground : IMPERIAL.textTertiary} />
            <Text style={{ fontSize: 12, fontWeight: '600', color: commitMsg.trim() ? IMPERIAL.primaryForeground : IMPERIAL.textTertiary }}>Commit</Text>
          </TouchableOpacity>
          <TextInput
            value={commitMsg}
      ...
      <Modal visible={showCloneModal} animationType="slide" transparent>
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', paddingHorizontal: 24 }}>
          <View style={{ backgroundColor: IMPERIAL.cardSolid, borderRadius: 16, borderWidth: 1, borderColor: IMPERIAL.border, padding: 20 }}>
            <Text style={{ fontSize: 16, fontWeight: '700', color: IMPERIAL.gold, textAlign: 'center', marginBottom: 16 }}>Clone Repository</Text>
            <TextInput
              value={cloneUrl}
            ...
            <View style={{ flexDirection: 'row', gap: 10 }}>
              <TouchableOpacity
                ...
                style={{ flex: 1, paddingVertical: 10, borderRadius: 10, borderWidth: 1, borderColor: IMPERIAL.border, alignItems: 'center' }}
              >
